...
namespace B12Test.Migrator
{
    public class MultiTenantMigrateExecuter : ITransientDependency
    {
        ...
        public bool Run(bool skipConnVerification)
        {
            ...
            // Explicitly using Abp.Extensions.StringExtensions to resolve ambiguity
            if (Abp.Extensions.StringExtensions.IsNullOrWhiteSpace(hostConnStr))
            {
            ...
            if (!skipConnVerification)
            {
                ...
                // Explicitly using Abp.Extensions.ObjectExtensions to resolve ambiguity
                if (!Abp.Extensions.ObjectExtensions.IsIn(command, "Y", "y"))
                {
            ...
            {
                _log.Write("An error occurred during migration of host database:");
            ...
            for (var i = 0; i < tenants.Count; i++)
            {
                ...
                if (!migratedDatabases.Contains(tenant.ConnectionString))
                {
                    ...
                    {
                        _log.Write("An error occurred during migration of tenant database:");
                        _log.Write(ex.ToString());
                ...
                {
                    _log.Write("This database has already migrated before (you have more than one tenant in the same database). Skipping it....");
                }
